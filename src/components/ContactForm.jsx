...
export default function ContactForm() {
  ...
  useEffect(() => {
    if (modalOpen) {
      ...
      // auto-close the modal after 4.5 seconds
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validate()) return;
    if (!API_URL) {
      setStatusText("API URL not configured. Set VITE_API_URL in .env file.");
      return;
    }
    ...
    <>
      ...
      {modalOpen && (
        ...
        >
          ...
          {/* modal panel */}
          <div className="relative bg-white rounded-lg max-w-md w-[90%] mx-auto p-6 shadow-xl z-10">
            <h2
            ...
            <div className="flex justify-end">
              <button
                ref={closeBtnRef}
                onClick={() => setModalOpen(false)}
                className="px-4 py-2 bg-[#F15D2B] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#F15D2B]/60"
              >
                Close
              </button>
            </div>
